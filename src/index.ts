...
interface CoinFundingRate {
    symbol: string;
    rate: number;
    timestamp: number; // Timestamp of the funding rate
}
    ...
    try {
        ...
        if (response.data.retCode === 0 && response.data.result && response.data.result.list) {
            return response.data.result.list
                .filter((instrument: BybitInstrumentsInfo) => instrument.contractType === 'LinearPerpetual') // Filter only for LinearPerpetual
